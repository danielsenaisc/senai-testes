...
public class PaisMarcaDAO extends AbstractDAO {
    ...
    private ArrayList<PaisMarca> montarObjeto(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            ...
            BigDecimal idPais = rs.getBigDecimal("pais_id");
            
            MarcaDAO marcaDao = new MarcaDAO();
            Marca marca = marcaDao.selectById(idMarca);
            
            PaisDAO paisDao = new PaisDAO();
            Pais pais = paisDao.selectById(idPais);
    ...
    public void insert(PaisMarca paisMarca) {
            String sql = String.format("INSERT INTO %s (marca_id, pais_id) VALUES (?, ?)", TABLE_NAME);
          
            ArrayList<Object> params = new ArrayList<Object>();
            
            params.add(paisMarca.getMarca().getId());
            params.add(paisMarca.getPais().getId());
            
            operacaoEscrita(sql, params);
    }

    public void delete(PaisMarca paisMarca) {
            String sql = String.format("DELETE FROM %s WHERE marca_id = ? and pais_id = ?",TABLE_NAME);
