...
public class MarcaDAO extends AbstractDAO {
	...
	private ArrayList<Marca> montarObjeto(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            ...
            Date dataCriacao = rs.getDate(Columns.dataCricacao);
            char genero = rs.getString(Columns.genero).charAt(0);
            short idadeInicial = rs.getShort(Columns.idadeInicial);
            short idadeFinal = rs.getShort(Columns.idadeFinal);
            char status = rs.getString(Columns.status).charAt(0);
    ...
    public void insert(Marca marca) {
            ...
            String sqlParam = String.format(sql, Tables.marca, Columns.nome, Columns.descricao, Columns.logomarca, Columns.dataCricacao, 
